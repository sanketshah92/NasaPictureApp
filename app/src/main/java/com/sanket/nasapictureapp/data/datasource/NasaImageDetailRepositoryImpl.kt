...
class NasaImageDetailRepositoryImpl(private val dataSource: NasaImageDataSourceImpl) :
    ...
    override suspend fun getImageData(): NasaImageData {
        lateinit var nasaImageData: NasaImageData
        val response = "[\n" +
                "    {\n" +
                ...
                "]"
        return try {
            //val response = dataSource.getNasaImages()
            nasaImageData = Gson().fromJson(
                response,
                NasaImageData::class.java
            )//response.body() ?: NasaImageData()
            nasaImageData
        } catch (e: Exception) {
            e.printStackTrace()
            NasaImageData()
